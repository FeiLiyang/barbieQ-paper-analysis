...
Sample_Groups <- function(barbieQ, size_group = 3) {
  ...
  repeat{
    ...
    sample_bb$sampleMetadata$LibSize <- ifelse(
      sample_bb$sampleMetadata$BarcodeSize >= 330000,
      "high", "low")

    # repeat if only one level in libSize
    if (length(unique(sample_bb$sampleMetadata$LibSize)) < 2L) next

    # model design matrix
    mydesign <- sample_bb$sampleMetadata %>%
      as.data.frame() %>%
      with(model.matrix(~0 + Group + LibSize))
  ...
  prop_asin <- testBarcodeSignif(
    barbieQ = sample_bb,
    sampleMetadata = sample_bb$sampleMetadata[,c("Group", "LibSize"),drop = FALSE],
