...
Sample_Groups <- function(barbieQ, size_group = 3) {
  ...
  repeat{
    ...
    # sample_bb$sampleMetadata$ps.Tissue <- ifelse(
    #   sample_bb$sampleMetadata$Tissue %in% c("Blood", "sac"),
    #   "Blood", "BM.SP")
    #
    # # repeat if only one level ps.Tissue
    # if (length(unique(sample_bb$sampleMetadata$ps.Tissue)) < 2L) next
    ...
    # case1: only one level in ps.Tissue; case2: ps.Tissue perfectly in line with grouping
  ...
  LFC_prop <- rowMeans(log2(assays(sample_X1)$proportion + 5e-7)) - rowMeans(log2(assays(sample_X2)$proportion + 5e-7))
  Gmean_prop <- rowMeans(log10(assays(sample_bb)$proportion + 5e-7))
