...
# Cluster correlating barcodes {.tabset .tabset-fade .tabset-pills}
...
```{r}
flag_cluster_BC <- monkeyHSPC_top@elementMetadata$barcodeCorrelatedCluster$cluster > 0

cluster_BC <- monkeyHSPC_top[flag_cluster_BC,]

cluster_group <- cluster_BC@elementMetadata$barcodeCorrelatedCluster$cluster

cluster_BC_prop <- cluster_BC@assays@data$proportion

## pivot to long
df_long <- as.data.frame(cluster_BC_prop) %>%
...
## add group info
df_long$cluster_group <- paste0("Cluster ", cluster_group[df_long$barcode])
## order samples
df_long <- df_long %>%
  group_by(cluster_group, sample) %>%
  mutate(sample_mean = mean(prop)) %>%
  ungroup() %>%
  group_by(cluster_group) %>%
  ## reorder samples.
  mutate(sample = forcats::fct_reorder(sample, prop, .fun = mean)) %>%
  ungroup()
## plot
ggplot(df_long, aes(x = sample, y = prop, group = barcode)) +
  geom_line(alpha = 0.7, aes(colour = cluster_group)) +
  geom_point(size = 0.8) +
