---
title: "barbieQ_paper_Figure3_FigureS4"
author: "Liyang Fei"
date: |
...
design_mixed$Subset <- factor(design_mixed$Subset, levels = c("null", "perturb"), labels = c("baseline", "perturbed"))
...
p_mixed <- plotBarcodeHistogram(mixed_dge, orderSamples = colnames(mixture_plus1_top)[order_sample])
...
p_mixed_illus <- p_mixed +
  ...
  scale_fill_manual(values = color_mapping$Subset) +
  labs(fill = "Subset") + 
...
Using [mixture_plus1_top](barbieQ_paper_S1.html) data, where an offset (one) was added to the raw count, based on which proportion was calculated. However the "top" barcodes were filtered based on the raw count data.
...
```
 ...
 - Diff_occ. when converting barcode presence / absence, setting the threshold as CPM>10 instead of the default 1, as an offset (one) was added to the raw count [preprocessing](barbieQ_paper_S1.html).
...
## F3G_left: Type I Error Rate as a function to N_BC
...
rbind(
  ...
  theme_bw()+
  theme_barbieQ_size() +
  theme(panel.grid.minor = element_blank(), axis.text.x = element_text(angle = 15), legend.position = "top",
        strip.placement = "outside", strip.text.y = element_text(size = 12),
        strip.text.x = element_blank(), strip.background.x = element_blank()) +
  scale_y_continuous(limits = c(0,0.2)) -> f3g1
...
ggplot(BC_set_prop_filter) +
  ...
  labs(color = "N_Barcodes", x = "Barcode Proportion", y = "Density", subtitle = "with updated proportion") +
  theme_barbieQ() +
  theme(aspect.ratio = 0.9) -> f3f1
```
...
ggplot(BC_set_prop_filter_raw) +
  ...
  coord_cartesian(xlim = c(1e-6, 3e-1)) +
  theme_linedraw() +
  labs(color = "N_Barcodes", x = "Barcode Proportion", y = "Density", subtitle = "with raw proportion") +
  theme_barbieQ() +
  theme(aspect.ratio = 0.9) -> f3f2
```
...
rbind(
  ...
  theme_bw() +
  theme_barbieQ_size() +
  theme(panel.grid.minor = element_blank(), axis.text.x = element_text(angle = 15), legend.position = "top", legend.key.size = unit(0.3, "cm"),
...
data.frame(x = (stats_barcodes_all_levels %>% filter(filter_level==0.95))$Amean_prop) %>%
  ...
  labs(x = "Quantiles of barcode mean proportion", y = "Barcode mean proportion",
       title = "Barcode proportion distribution", subtitle = " ") -> f3d

f3d
```

## F3E_left: False positives in quantiles
...
plot_fdr_on_quantile(stats_barcodes_all_levels %>% filter(filter_level==0.95), num_quantiles = 10) + 
  theme_barbieQ() +
  labs(title = "Type I error rate in quanties") -> f3e1
```
...
```{r}
colnames(mixture_plus1_top)

prop_raw <- mixture_plus1_top@assays@data$proportion
...
```{r}
mixture_plus1_top$sampleMetadata %>% as.data.frame() %>% summary()

mixed <- mixture_plus1_top[, mixture_plus1_top$sampleMetadata$Subset %in% c("perturb", "null")]
...
fs4a <- (p_es_de_asin + theme(legend.title = element_text(size = 12), legend.text = element_text(size = 11),
        axis.title = element_text(size = 13), axis.text = element_text(size = 12)) ) / 
...
fs4a
```
## F3G_right: TPR as a function to N_BC
...
rbind(
  ...
  theme(panel.grid.minor = element_blank(), axis.text.x = element_text(angle = 15), legend.position = "top",
        strip.placement = "outside", strip.text.y = element_text(size = 12),
        strip.text.x = element_blank(), strip.background.x = element_blank()) + guides(color = "none") -> f3g2

f3g2
```


## F3E_right: TPR in quantiles
...
plot_tpr_on_quantile(test_all_levels %>% filter(filter_level==0.95), num_quantiles = 10) +
  theme_barbieQ() +
  labs(title = "Power (true positive rate) in quanties") -> f3e2
```
...
p_resid_qq_35 <- ggplot(resi_df %>% filter(Contrast == "0.35 - 0"), 
  ...
  theme_bw() +
  theme(legend.position = "none", strip.text = element_blank(), plot.title.position = "panel")
```

```{r, fig.width=8, fig.height=9}
fs4b <- (p_resid_distr_35 + 
...
f3 <- ((wrap_elements(full = f3a) + theme(plot.margin = unit(c(0,0,0,0), "cm"))) + 
    ...
    wrap_elements(f3e1 + theme(legend.position = "top", plot.margin = unit(rep(0,4), "cm"))) +
    wrap_elements(f3e2 + theme(legend.position = "top", plot.margin = unit(rep(0,4), "cm")) + guides(color = "none")) +
    wrap_elements(f3f + theme(legend.position = "top", plot.margin = unit(c(0,0,0,0), "cm"))) +
    wrap_elements(f3g1 + theme(legend.position = "top",plot.margin = unit(rep(0,4), "cm"))) +
    wrap_elements(f3g2 + theme(legend.position = "top",plot.margin = unit(rep(0,4), "cm"))) 
...
"
fs4 <- (
    wrap_elements(fs4a + theme(plot.margin = unit(rep(0,4), "cm"))) + 
    wrap_elements(fs4b + theme(plot.margin = unit(c(0,0,0,0), "cm")))
    )+ 
...
Saving this figure in [fs4](../output/fs4.png)